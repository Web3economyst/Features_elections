...
def preparar_base():
    """Carrega brutos 2022, casa features por numero_registro<->ID, calcula erro e TODAS as features.

    AJUSTES vs 2018 (so o necessario):
      - chave do merge: 2022 casa por 'numero_registro' (BR-xxxxx/2022) <-> 'ID'.
      - percentual real do TSE ja vem embutido na base long 2022
        (percentual_pesquisa_valido e percentual_valido); sem dicionario de reais.
      - pesquisas de 2021 / pos-eleicao saem via dias_ate_eleicao >= 0.
    Todas as VARIAVEIS geradas sao as mesmas do modelo de 2018.
    """
    LONG = r'C:\Users\Games\OneDrive\Desktop\osfstorage-archive\Features_2018\Novo\base_2022_TODAS_PARA_MERGE_Sheet1_.csv'
    FEAT = r'C:\Users\Games\OneDrive\Desktop\osfstorage-archive\Features_2018\Novo\bases_2022_dias_corrigido_final_in_.csv'

    df   = pd.read_csv(LONG, encoding='latin1', dtype=str)
    feat = pd.read_csv(FEAT, sep=';', encoding='latin1', dtype=str)

    df['numero_registro'] = df['numero_registro'].astype(str).str.replace('\u2010','-').str.strip()
    feat['ID']            = feat['ID'].astype(str).str.replace('\u2010','-').str.strip()

    for c in ['percentual','percentual_pesquisa_valido','percentual_valido',
              'dias_ate_eleicao','quantidade_entrevistas','turno_real']:
        df[c] = pd.to_numeric(df[c], errors='coerce')

    b = df[df['categoria_voto_corrigida'] == 'candidato'].copy()
    b = b[b['ano'].astype(str) == '2022'].copy()
    b = b[b['dias_ate_eleicao'] >= 0].copy()


    #pct pesquisas
    b['pct']   = b['percentual'].apply(lambda x: x*100 if pd.notna(x) and x <= 1 else x)
    b['turno'] = b['turno_real']


    #percentual_valido
    b['percentual_valido'] = b['percentual_pesquisa_valido'] * 100

    #percentual_real
    b['percentual_real']   = df.loc[b.index, 'percentual_valido'] * 100

    #vies
    b['vies']     = b['percentual_valido'] - b['percentual_real']

    #abs_vies - Dependnte
    b['abs_vies'] = b['vies'].abs()


    #soma_rt
    b['soma_rt'] = b.groupby(['numero_registro','turno'])['pct'].transform('sum')
    ...
    # --- (AJUSTE 2a) mantem apenas o CENARIO 1 (todas as variantes ligadas ao "1") ---
    #     robusto a maiuscula/acento/espacamento/ordem: ancora so em "cenario N".
    if 'descricao_cenario' in b.columns:
        num_cen = b['descricao_cenario'].map(
            lambda s: int(mt.group(1)) if (mt := re.search(r'cen[aá]rio\s+(\d+)', str(s), re.I)) else 1)
        n_antes = len(b); b = b[num_cen == 1].copy()
        print(f"[cenario 1] removidas {n_antes - len(b)} linhas de cenarios secundarios (restam {len(b)})")

    # --- (AJUSTE 2b) mantem apenas candidatos com voto real >= 3% (remove < 3%) ---
    n_antes = len(b); b = b[b['percentual_real'] >= 3].copy()
    print(f"[>=3%] removidas {n_antes - len(b)} linhas de candidatos < 3% (restam {len(b)})")

    # --- (AJUSTE 1) remove linhas com NA nas variaveis usadas na modelagem ---
    #     substitui o antigo filtro que so olhava abs_vies.notna().
...
if __name__ == '__main__':
    ...
    # Exportar
    with pd.ExcelWriter('resultados_meus_testes_2022.xlsx', engine='openpyxl') as w:
    ...
    print("\nExportado: resultados_meus_testes_2022.xlsx")
